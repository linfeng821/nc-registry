...
import cn.lf.nacos.netty.ChannelHandlerHolder;
import cn.lf.nacos.netty.ConnectionListener;
...
abstract public class ConnectionWatchDog extends ChannelInboundHandlerAdapter implements TimerTask, ChannelHandlerHolder {
    ...
    public ConnectionWatchDog(Bootstrap bootstrap, Timer timer, DiscoverProperties discoveryProperties, NamingServiceImpl nacosNamingService, boolean reconnect) {
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        if (reconnect) {
            ...
            if (attempts >= 5) {
                ...
                String serverIp = NettyClient.getKeyBuValue(NettyClient.nettyServer);
                ...
                if (NettyClient.nettyServers.size() > 0) {
                    ...
                    attempts = 0;
                    log.warn("尝试连接" + NettyClient.getKeyBuValue(NettyClient.nettyServer));
                } else {
    ...
    public void run(Timeout timeout) throws Exception {
        ...
        log.info("重新连接上了" + NettyClient.getKeyBuValue(NettyClient.nettyServer));
        //重新连接后channel会改变
        NettyClient.channel = channelFuture.channel();
        //System.out.println("channel:"+NettyClient.channel);

        //因为防止只有一个server节点，所以重新注册服务，要去重!!!
        nacosNamingService.registryInstance(discoveryProperties.getClientIp(), discoveryProperties.getClientPort(), discoveryProperties.getClusterName(), discoveryProperties.getService(), discoveryProperties.getNamespace());
